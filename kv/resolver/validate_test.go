...
func TestContainsReferences_Both(t *testing.T) {
	...
	cases := []struct {
		...
		{"bare whole-value string, not JSON", `kv://vault/db#password`, true},
		{"bare inline token, not JSON", `$kv{env:HOST}`, true},
		{"whole-value marker mid-string still counts", `{"note":"see kv://s/p for details"}`, true},
		{"unclosed inline marker does not count", `{"x":"$kv{env:X"}`, false},
		{"random string containing $kv{ does not count", `{"token":"a$kv{9fZ2qLpX"}`, false},
		{"inline braces without a store separator do not count", `{"x":"$kv{HOME}"}`, false},
		{"empty inline braces do not count", `{"x":"$kv{}"}`, false},
		{"kv scheme without a path does not count", `{"x":"kv://store"}`, false},
